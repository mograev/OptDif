"""
Train a GBO model on a subset of PCA components.
"""
...
gbo_train_group.add_argument("--n_pca_components", type=int, default=512, help="number of PCA components to use")
...
def train_gbo(
    ...
    dataset = TensorDataset(torch.tensor(Z_train, dtype=torch.float32), torch.tensor(y_train, dtype=torch.float32).unsqueeze(1))
