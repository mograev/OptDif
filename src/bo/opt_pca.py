...
opt_group.add_argument("--n_pca_components", type=int, default=512, help="Number of PCA components to use for dimensionality reduction (if applicable)")
...
def robust_multi_restart_optimizer(
    ...
    Args:
        ...
        n_pca_components (int): Number of PCA components to use for dimensionality reduction.
    Returns:
    ...
    if sample_distribution == "uniform":
        ...
        latent_grid = X_train[np.random.choice(X_train.shape[0], n_samples, replace=False)]
    else:
        raise NotImplementedError(sample_distribution)
    
    # Reduce dimensionality of PCA components
    latent_grid_init = latent_grid.copy()
    latent_grid_fixed = latent_grid_init.copy()[:, n_pca_components:]
    latent_grid = latent_grid[:, :n_pca_components]
    logger.debug(f"latent_grid shape after PCA reduction: {latent_grid.shape}")
    # Reduce dimensionality of training data accordingly
    X_train = X_train[:, :n_pca_components]
    ...
    if opt_constraint_threshold is None:
        ...
        z_valid = np.array([z for i, z in enumerate(latent_grid) if logdens_z_grid[i] > opt_constraint_threshold],
                            dtype=np.float32)
        # Get indices of valid points and reduce grids accordingly
        valid_indices = np.where(logdens_z_grid > opt_constraint_threshold)[0]
        latent_grid_init = latent_grid_init[valid_indices]
        latent_grid_fixed = latent_grid_fixed[valid_indices]
    ...
    # Sort the fixed latent grid according to the sorted valid points
    latent_grid_init = latent_grid_init[z_valid_prop_argsort]
    latent_grid_fixed = latent_grid_fixed[z_valid_prop_argsort]
    ...
    latent_grid_fixed = latent_grid_fixed[indices]

    # Append the initial fixed components to the candidates
    x_candidates = np.hstack((x_candidates, latent_grid_fixed))
    ...
    return x_candidates[:num_pts_to_return], opt_vals_candidates[:num_pts_to_return], latent_grid_init[:num_pts_to_return]
...
def opt_main(args):
    ...
    # Reshape the data, but keep the original shape for later
    ...
    if args.surrogate_type == "GP":
        ...
        mu, var = surrogate.predict(latent_pred[:, :args.n_pca_components])
    logger.info("mu at points:")
    logger.info(list(mu.ravel()))
    logger.info("var at points:")
    logger.info(list(var.ravel()))

    # Inverse transform the latent predictions back to the original space
    ...
    latent_grid_init = latent_grid_init.reshape(latent_grid_init.shape[0], *x_shape)
    latent_pred = latent_pred.reshape(latent_pred.shape[0], *x_shape)
    logger.info(f"latent_pred shape after reshaping: {latent_pred.shape}")
