...
from src import DNGO_TRAIN_FILE, GP_TRAIN_FILE, OPT_FILE
...
# BO arguments
def add_bo_args(parser):
    """ Add arguments for bayesian optimization """

    # BO arguments (used for both DNGO and GP)
    bo_group = parser.add_argument_group("BO")
    bo_group.add_argument("--bo_surrogate", type=str, default="GP", help="Surrogate model to use for BO: 'GP' or 'DNGO'")
    bo_group.add_argument("--n_out", type=int, default=5, help="Number of points to return from optimization")
    bo_group.add_argument("--n_starts", type=int, default=20, help="Number of optimization runs with different initial values")
    bo_group.add_argument("--n_rand_points", type=int, default=8000, help="Number of random points to sample for surrogate model training")
    bo_group.add_argument("--n_best_points", type=int, default=2000, help="Number of best points to sample for surrogate model training")
    ...
    bo_group.add_argument("--opt_method", type=str, default="SLSQP", help="Optimization method to use: 'SLSQP', 'COBYLA' 'L-BFGS-B'")
...
# Setup logging
logger = logging.getLogger("lso-ffhq-bo")
...
def _decode_and_predict(latent_model, sd_vae, z, device):
    ...
    # Clamp to [-1, 1] range
    decoded_images_normalized = torch.clamp(decoded_images_normalized, -1.0, 1.0).cpu()

    # Calculate objective function
    predictions = []
    for i in range(len(decoded_images_normalized)):
        # Calculate the red percentage
        red_score = calculate_red_percentage(decoded_images_normalized[i])

        # Append the red score to the list
        predictions.append(red_score)

    # Convert predictions to numpy
    predictions = np.array(predictions)

    return decoded_images_normalized, predictions, sd_latents


def latent_optimization(args, latent_model, sd_vae, datamodule, num_queries_to_do, bo_data_file, bo_run_folder, device="cpu", pbar=None, postfix=None):
...
def main_loop(args):
    ...
    latent_model = LatentVQVAE(
        ...
        n_embed=latent_model_config["embedconfig"]["n_embed"],
        embed_dim=latent_model_config["embedconfig"]["embed_dim"],
        ckpt_path=args.latent_model_ckpt_path,
        ignore_keys=["model_ema"],
    ...
    with tqdm(
        ...
        for ret_idx in range(num_retrain):
            ...
            if not os.path.exists(str(Path(data_dir) / f"sampled_data_iter{samples_so_far}")):
                ...
                os.makedirs(str(Path(data_dir) / f"sampled_data_iter{samples_so_far}/latents"))
                os.makedirs(str(Path(data_dir) / f"sampled_data_iter{samples_so_far}/sd_latents"))
                os.makedirs(str(Path(data_dir) / f"sampled_data_iter{samples_so_far}/img_tensor"))
            ...
            for i, z in enumerate(z_query):
                ...
                torch.save(z, str(Path(data_dir) / f"sampled_data_iter{samples_so_far}/latents/tensor_{i}.pt"), pickle_protocol=pickle.HIGHEST_PROTOCOL)

            # Save the new sd latent points
            for i, sd in enumerate(sd_query):
                if not isinstance(sd, torch.Tensor):
                    sd = torch.from_numpy(sd)
                torch.save(sd, str(Path(data_dir) / f"sampled_data_iter{samples_so_far}/sd_latents/tensor_{i}.pt"), pickle_protocol=pickle.HIGHEST_PROTOCOL)
                new_filename_list.append(str(Path(data_dir) / f"sampled_data_iter{samples_so_far}/sd_latents/tensor_{i}.pt"))
            
            # Save the new images
            for i, x in enumerate(x_new):
                if not isinstance(x, torch.Tensor):
                    x = torch.from_numpy(x)
                torch.save(x, str(Path(data_dir) / f"sampled_data_iter{samples_so_far}/img_tensor/tensor_{i}.pt"), pickle_protocol=pickle.HIGHEST_PROTOCOL)
