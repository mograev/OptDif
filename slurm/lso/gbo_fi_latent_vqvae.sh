...
#SBATCH --job-name=lso_gbo_fi_latent_vqvae               # Job name
#SBATCH --output=logs/lso/gbo_fi_latent_vqvae_01_%j.out  # Output log file
#SBATCH --error=logs/lso/gbo_fi_latent_vqvae_01_%j.err   # Error log file
...
n_init_retrain_epochs=0.05 #1
result_path="results/gbo_fi_latent_vqvae_02/"
sd_vae_path="stabilityai/stable-diffusion-3.5-medium"
latent_model_config_path="models/latent_vqvae/version_8/hparams.yaml"
latent_model_ckpt_path="models/latent_vqvae/version_8/checkpoints/last.ckpt"
...
# Optimization
opt_strategy="GBO_FI" # "GBO", "GP", "DNGO", "GBO_PCA", "GBO_FI"
n_out=5 #5
n_starts=20 #20
...
CUDA_VISIBLE_DEVICES=3 python src/lso_latent_vqvae.py \
    --device $device \
    --seed $seed \
    --img_dir $img_dir \
